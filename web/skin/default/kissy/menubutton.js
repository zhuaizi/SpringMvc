    ...
    if(v) {
      ...
      if(!menu.get("visible")) {
        ...
        if(self.get("matchElWidth")) {
          ...
          var borderWidth =0;
          if (self.get("borderWidth")) {
        	borderWidth =(parseInt(menuEl.css("borderLeftWidth")) || 0) + (parseInt(menuEl.css("borderRightWidth")) || 0);
          }
...
KISSY.add("menubutton/select", ["node", "./control"], function(S, require) {
  ...
  var Select = MenuButton.extend({bindUI:function() {
    ...
    _updateCaption(this)
  }}, {ATTRS:{value:{},nameid:{value :false}, defaultCaption:{value:""}, collapseOnClick:{value:true}}, decorate:function(element, cfg) {
    ...
    var nid = name;
    name = nid+"_txt"; 
	if(select.get('nameid')){name = nid;nid = nid+"_id"}
      var input = (new Node("<input" + ' type="hidden"' + ' name="' + nid + '" value="' + curValue + '">')).insertBefore(element, undefined);
       var curctx = ''; 
     if(selectedItem){curctx = selectedItem.content;}
      var data_rules = element.attr("data-rules");
    if(data_rules){data_rules=' data-rules="'+data_rules+'"';}else{data_rules='';}
    var data_msg = element.attr("data-msg");
    if(data_msg){data_msg=' data-msg="'+data_msg+'"';}else{data_msg='';}
    var input2 = (new Node("<input" + ' type="hidden"' + ' name="' + name + '" '+data_rules+data_msg+' value="' + curctx + '">')).insertBefore(element, undefined);
      select.on("afterValueChange", function(e) {
        input.val(e.newVal || "");
        var t = e.target;
		input2.val(t.get("content")|| "");
      })
    }
    element.remove();
    return select
