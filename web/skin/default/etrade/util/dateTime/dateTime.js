...
//            		self.input.attr("readonly","readonly");
        ...
        _buildParam: function (o) {
            ...
            S.each({
                ...
                closable: true, //在弹出情况下, 点选日期后是否关闭日历, 默认为false
...
//            self.hours = date.getHours();//小时
//            self.minutes = date.getMinutes();//分钟
//            self.seconds = date.getSeconds();//秒
            return this;
        },
        /**
         * 处理日期TO字符串
    ...
    S.augment(Calendar, {
        ...
        Page:function (config, father) {
            ...
            this.html = [
                ...
                '<a href="javascript:void(0);" class="ks-cal-footer-select-btn {$showtime}">确定</a>',
                '<a href="javascript:void(0);" class="ks-cal-footer-clear-btn">清除</a>',
                '</div>',
                '</div>'
            ].join("");
            this.nav_html = [
    ...
    S.augment(Calendar, {
        ...
        TimeSelector:function(ft, father) {
            ...
            this.ctime = Node('<div class="ks-cal-time ks-cal-time-cust">'+
            					'<font class="ks-cal-time-str">时间：</font>'+
