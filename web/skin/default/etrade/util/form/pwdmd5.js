 ...
 * 支付密码+验证MD5转换
 */
KISSY.add(KISSY.myPackageName + "/util/form/pwdmd5", function(S, Base, DOM,
		Event,Cookie) {

	var Pwdmd5 = function(config) {
		Pwdmd5.superclass.constructor.call(this, config);
	};

	S.extend(Pwdmd5, Base);// 继承基类

	Pwdmd5.ATTRS = {
	};

	Pwdmd5.prototype.render = function() {
		
	};
	/***
	 * 传入 password 和 code 可获得加密后的串
	 */
	Pwdmd5.getsafepwd =function(str1, str2) {
		try {
			var s3 = Cookie.get("ys-SF_VC_ID");
			return _safepwd(str1, str2, s3);
		} catch (e) {
