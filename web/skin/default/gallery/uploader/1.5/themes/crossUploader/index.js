...
gallery/uploader/1.5/themes/crossUploader/index
...
/**
 * @fileoverview 支持手机上传的主题
 * @author 明河
 **/
KISSY.add('gallery/uploader/1.5/themes/crossUploader/index',function (S, Node, ImageUploader) {
    var EMPTY = '', $ = Node.all;
     ...
     * @author 明河
     */
    function CrossUploader(config) {
        var self = this;
        //调用父类构造函数
        CrossUploader.superclass.constructor.call(self, config);
    }

    S.extend(CrossUploader, ImageUploader, /** @lends CrossUploader.prototype*/{
        /**
        ...
        render: function () {
            var self = this;
            CrossUploader.superclass.render.call(self);
            var uploader = self.get('uploader');
            var $target = uploader.get('target');
            var $btnWrapper = $target.parent();
            var $uploaderBtn = $btnWrapper.all('.crossUploader-button');
            var uploaderBtnTop = $uploaderBtn.css('top');
            var $mobileUploaderBtn = $btnWrapper.all('.mobile-upload-button');
    ...
    }, {ATTRS: /** @lends CrossUploader.prototype*/{
        /**
         *  主题名（文件名），此名称跟样式息息相关
         * @type String
         * @default "CrossUploader"
         */
        name: {value: 'crossUploader'},
        /**
         * 队列使用的模板
         * @type String
         * @default ""
